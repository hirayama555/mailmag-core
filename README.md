...
- **読者向け紹介LP** — ヒーロー／特徴／3ステップ／登録フォームを備えた集客用の独立ランディングページ。スクロール連動の登場エフェクト（AOS）付き（v1.6.0+）
...
- **バウンス自動処理（メールパイプ）** — 不達通知をメールパイプで受信し、ハードバウンスの購読者を自動でエラー停止（v1.4.0+）
- **バウンス自動処理（IMAPポーリング）** — メールパイプが使えないサーバー向けの代替方式。bounce 専用メールボックスを cron で定期取得・処理（v1.5.0+）
...
最新リリース: [**v1.6.0**](https://github.com/hirayama555/mailmag-core/releases/latest)（読者向け紹介LP + スクロール登場エフェクト）

各クライアントの稼働バージョンは `core/VERSION` で確認できます。

直近の主な変更点:

- **v1.6.0** — **読者向け 紹介ランディングページ（独立LP）**を追加（`landing.php`）。`register.php`（登録フォーム単体）とは別に、メールマガジンの魅力を訴求して登録へ導く集客用ページ。ヒーロー／特徴／3ステップ／数字／登録フォーム／フッターで構成し、**各セクションの登場エフェクト**を AOS（Animate On Scroll, CDN・ビルド不要）で実装（画面に入った瞬間に `fade-up`／`fade-right`／`zoom-in` で段階的＝スタッガー登場）。あわせてヒーロー背景のグラデーション移動、スクロールで凝縮する固定ヘッダー、数字のカウントアップ（`IntersectionObserver` 自前実装）、スムーススクロールを実装。登録処理は同一セッションの CSRF トークンで `register.php` に POST して**一本化**（既存のレート制限・enumeration 対策・確認メール送信を再利用）。`prefers-reduced-motion` 配慮＋AOS 不達時のフォールバック表示つき。LP の HTML/CSS は `core/app/landing.php` に内包したので**自動更新対象**だが、**既存クライアントは初回のみ `landing.php`（ルートシェル）の手動アップロードが必要**。本文コピーは `<!-- ▼編集 -->` 箇所で差し替え可。
...
- [x] **IMAPポーリングによるバウンス処理**（v1.5.0、メールパイプ不可環境向けの代替方式）
