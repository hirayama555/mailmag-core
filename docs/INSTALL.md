...
mailmag は `mail()` 経路で送信するため、レンタルサーバ側で OpenDKIM 等が動いていない限り
DKIM 署名は付きません。**SPF alignment が成立すれば DMARC=pass は単独で達成可能**な設計ですが、
DKIM 署名がある方が iCloud Mail 等のより厳格な分類器で有利です。サーバ管理者に確認してください。
