...
## 7-1. バウンス（配信不達）処理の設定（任意・推奨）

配信したメールが宛先不明などで届かなかった場合、メールサーバーは「バウンス通知
（不達メール）」を**送信元（Envelope-From = 送信元メールアドレス）**へ返します。
MailMag はこの通知をメールパイプで受け取り、**恒久エラー（ハードバウンス）した宛先の
購読者を自動的に「エラー停止」**にできます。これにより無効アドレスへの配信を止め、
送信ドメインの評価（レピュテーション）低下を防げます。

レンタルサーバーのメール管理画面で、**送信元メールアドレス宛のメール**を以下のコマンドへ
パイプ転送するよう設定してください（空メール登録と同じ「コマンド転送」機能を使います）:

```
転送先: |/usr/local/bin/php /path/to/mailmag/bounce.php
```

- バウンス通知は送信元アドレス（config の `FROM_EMAIL` / 設定画面の送信元）に戻るため、
  そのアドレスに対してコマンド転送を設定するのが基本です。通常のメールも受け取りたい場合は、
  サーバーによっては「転送（コピー）」で受信箱とコマンドの両方へ配送できます。
- **ハードバウンス（恒久エラー: ユーザー不明など 5.x.x）のみ**購読者を「エラー停止」にします。
  ソフトバウンス（一時エラー: メールボックス満杯など 4.x.x）は記録のみで購読者は維持します。
- 処理結果は `data/bounce.log` に記録されます。停止された件数はダッシュボードの
  「エラー停止」および購読者一覧のステータスで確認できます。
- `bounce.php` はメールパイプ専用です（`.htaccess` で HTTP 直接アクセスは禁止済み）。

> 💡 **既存環境を更新する場合のみ**、`bounce.php`（ルートシェル）を1回だけ手動で FTP
> アップロードしてください（自動更新は `core/` のみで、ルートの `*.php` は対象外のため）。
> 新規インストールでは `client-template` に同梱されています。
...
## 10. ビジュアルエディタと画像ライブラリ（v1.3.0+）

「メール送信」画面で「HTMLメールとして送信する」をチェックすると、HTML本文欄が
**ビジュアルエディタ（WYSIWYG）**になります。太字・色・リンク・箇条書きなどをマウス操作で
編集でき、HTML を直接書く必要はありません（エディタが読み込めない環境では従来どおり
HTML を直接記述することも可能です）。

ツールバーの画像ボタンから、**事前にアップロードした画像をライブラリから選んで挿入**できます。
画像はメール受信側のメーラーが直接読み込むため、公開ディレクトリ `uploads/` に保存されます。

- 画像配信は **公開ディレクトリ `uploads/`** が担います。**新規インストールでは
  `client-template` に同梱**されているため追加作業は不要です（初回アップロード時に
  `uploads/` と保護用 `.htaccess` がエンドポイントにより自動生成されます）。
- 画像管理エンドポイントは `media.php`（ルートの薄いシェル）です。**既存環境を v1.3.0 へ
  更新する場合のみ**、`media.php` を1回だけ手動で FTP アップロードしてください
  （コア自動更新は `core/` のみを更新し、ルートの `*.php` は更新しないため）。
- アップロード可能な形式は **JPEG / PNG / GIF / WebP**、上限は **1ファイル 5MB・合計 100MB** です。
- `uploads/.htaccess` は PHP 等のスクリプト実行を禁止します（画像を装ったスクリプトの実行を防止）。

> 💡 **ビジュアルエディタについて**: エディタ本体（TinyMCE）は管理者のブラウザが
> CDN から読み込みます（APIキー不要・無料）。管理画面を操作する端末がインターネットに
> 接続できる必要があります。メールの送信処理やレンタルサーバー側には影響しません。

> ⚠️ **メールでの画像の見え方**: 多くのメールクライアントは既定で画像をブロックします。
> 受信者が「画像を表示」を押すまで表示されないため、画像に頼り切らないレイアウトを推奨します。
