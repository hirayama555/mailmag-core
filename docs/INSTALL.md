...
## 7-2. バウンス処理（IMAPポーリング方式・v1.5.0+）

メールパイプ（コマンド転送）が使えないサーバー（カゴヤ KIR 等の仮想メールアカウントなど）
向けの代替方式です。**7-1 が使える環境では 7-1 を推奨**します。

### 概要

バウンス専用メールアドレス（例: `bounce@example.net`）を作成し、IMAP で定期取得・処理します。
メールパイプの代わりに **cron** がバウンス受信箱を5分ごとに確認し、不達購読者を自動停止します。

### 設定手順

**1. バウンス専用メールアドレスを作成する**

レンタルサーバーのメール管理画面でバウンス受信専用のメールアドレスを新規作成します。
（例: `bounce@example.net`）このアドレス宛に転送を設定する必要はありません。受信箱に溜まった
バウンス通知を cron が定期的に読み出して処理します。

> ⚠️ このメールアドレスを Envelope-From（送信元）にする必要はありません。
> MailMag は config.php の `FROM_EMAIL` を Envelope-From に使います。
> バウンス通知は通常 `FROM_EMAIL` 宛に戻ってきますが、受信だけを bounce 専用アドレスに
> 向けたい場合はそのアドレスへの転送（コピー）を設定してください。

**2. MailMag 管理画面で IMAP 設定を行う**

「システム設定 → IMAP受信設定（バウンスポーリング）」で以下を入力して保存します:

| 項目 | 値の例 | 備考 |
|---|---|---|
| IMAPを有効にする | ✓ | |
| IMAPサーバー | `mail.example.net` | サーバーの「メールソフト設定」を参照 |
| ポート | `993` | SSL/TLS の場合（推奨） |
| 暗号化 | `SSL/TLS (993)` | |
| IMAPユーザー名 | `bounce@example.net` | 作成したメールアドレス |
| IMAPパスワード | （メールのパスワード） | 変更時のみ入力 |

保存すると自動で接続テストが走ります。成功すればアラートが表示されます。

**3. `cron_bounce_imap.php` を手動でアップロードする**

このファイルはルートの薄いシェルのため、**既存環境では1回だけ手動アップロードが必要**です
（自動更新は `core/` のみ対象）。新規インストールでは `client-template` に同梱されているため
追加作業は不要です。

```
FTP でアップロード: client-template/cron_bounce_imap.php → /path/to/mailmag/cron_bounce_imap.php
```
...
```cron
# 5分ごと: IMAP バウンスポーリング
*/5 * * * * /usr/local/bin/php /path/to/mailmag/cron_bounce_imap.php
```

カゴヤの場合のパス例:
```cron
*/5 * * * * /usr/bin/php /home/<アカウント名>/public_html/<MailMag設置パス>/cron_bounce_imap.php
```

**処理結果の確認**

- ログ: `data/bounce.log` に `HARD` / `SOFT` / `ERROR` が記録されます
- 停止された購読者: 管理画面「購読者一覧」のステータスが「エラー停止」になります

> 💡 **メールパイプ版（bounce.php）との共存**: 両方を有効にしても問題ありません。
> 環境が対応している場合はメールパイプ版を優先してください（リアルタイム処理のため）。
> IMAP ポーリング版はポーリング間隔（最大5分）の遅延がありますが、機能は同等です。
...
## 11. 読者向け 紹介ランディングページ（v1.6.0+）

`register.php`（登録フォーム単体）とは別に、メールマガジンの魅力を訴求して登録へ導く
**集客用の独立ランディングページ `landing.php`** を用意しています。ヒーロー・特徴・
登録の3ステップ・登録フォームで構成され、各セクションは**スクロールに連動して
ふわっと登場**します（AOS = Animate On Scroll を CDN から読み込み。ビルド不要）。

- 公開 URL は `https://（設置先）/landing.php` です。SNS 広告やプロフィールのリンク先など、
  集客の入口として利用してください。登録フォームの送信は `register.php` に引き継がれ、
  確認メール送信・レート制限などの既存ロジックがそのまま適用されます。
- **新規インストールでは `client-template` に同梱**されているため追加作業は不要です。
- **既存環境を v1.6.0 へ更新する場合のみ**、`landing.php`（ルートの薄いシェル）を1回だけ
  手動で FTP アップロードしてください（コア自動更新は `core/` のみを更新し、ルートの
  `*.php` は更新しないため。LP の中身＝HTML/CSS は `core/app/landing.php` にあり自動更新されます）。
- **本文コピーの編集**: 見出し・特徴・数字などのサンプル文言は `core/app/landing.php` 内の
  `<!-- ▼編集 -->` コメント箇所で差し替えられます（`core/` は自動更新対象のため、更新時に
  上書きされる点に注意。文言を恒久的に固定したい場合はクライアント側で別ページ化を検討）。

> 💡 **登場エフェクトについて**: アニメーション本体（AOS）は閲覧者のブラウザが CDN から
> 読み込みます。CDN に到達できない場合や JavaScript 無効時でも、各セクションは
> 通常表示にフォールバックして内容が欠けません。OS で「視差効果を減らす／動きを減らす」
> を有効にしている利用者にはアニメーションを自動的に抑制します（アクセシビリティ配慮）。
